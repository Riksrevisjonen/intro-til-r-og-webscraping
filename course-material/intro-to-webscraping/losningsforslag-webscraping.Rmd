...
1. Lag et element av typen p med klassen mitt-element og skriv tekst inn idenne
2. Lag et nytt element av typen ul med klassen min-liste
3. Lag minst tre listeelementer inne i elementet du opprettet i forrige steg. Skrivtekst inn i hvert listeelement
...
I et større skript som skal gjenbrukes på et stort antall sider, vil vi også ønske å bygge inn litt enkel feilhåndtering, for eksmpel med å sjekke statuskoden vi får i responsobjektet fra `httr` eller teste om selve forespørselen feiler med `try` eller `tryCatch`.
