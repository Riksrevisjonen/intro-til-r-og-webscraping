...
* Finn hvilke elementer som inneholder de data vi er interessert i
...
- Det er derfor *ikke*  sikkert at alt inneholdet på nettsiden er inkludert i HTML kildekoden `r emo::ji("sad")`
...
Med `rvest` vil *ikke* finne noe særlig på nettsiden valgresultat.no, siden nesten hele siden generes av at `Javacript`. 
...
- Noen nettsteder vil også kunne blokkere forespørselen din dersom du ikke er en nettleser 

>- Hvordan skal vi da få tak i inneholdet?  `r emo::ji('thinking')`
...
Løser utfodringen med lese inn Javascript, samt at det blant annet åpner muligheter for å klikke på elementer på en nettside.    
...
For å kunne bruke Selenium trenger vi imidlertid en server denne kan kjøre på. 
...
docker run -d -p 4445:4445 --shm-size 2g selenium/standalone-firefox
```
...
```{r echo=TRUE , eval=FALSE}
# Opprett en tilkobling til Selenium-serveren  
remDr <- remoteDriver(remoteServerAddr = Sys.getenv('SEL_HOST'), port = 4445L)
...
h <- readRDS('assets/sel-example.RDS')
```
