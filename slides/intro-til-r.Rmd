...
```{r, echo=TRUE}
eksp <- c('Tor', 'Stig', 'Annette', 'Åse Kristin', 'Tora', 'Sverre', 'Aleksander')
eksp[c(1,4,6)]  # Elementene 1, 4 og 6
length(eksp)    # Bruk `length()` for å tak lengden på en vektor
eksp[length(eksp)] # Det siste elementet
```
...
## Inspiser data
Datasett: mtcars - 32 biler og 10 ulike egenskaper ved de
```{r, echo=TRUE}
head(mtcars)
```
...
if(x >= 10){
  print('X er større enn eller lik 10')
} else {
  print('X er ikke større enn eller lik 10')
}
