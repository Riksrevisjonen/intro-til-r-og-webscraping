...
- Funksjoner kan ha en eller flere _argumenter_
...
## Oppgave 1 {.flexbox .vleft .extraspace}
...
* Sjekk ut funksjonen `sample`. Hvilke argumenter tar funksjonen?
* Lag en vektor `my_vec` med 5 tilfeldig heltall trukket fra en vektor med alle tallene fra `1` til `10`. 
...
## Bruk av Rs innebygde `return` funksjon  
...
Et annet viktig poeng når det gjelder funksjoner er at objekter som defineres inne i en funksjonen kun er tilgjenglig inne i funksjonen. Eksempelvis vil objektet `out` **ikke** være tilgjengelig utenfor funksjonen `square`.  
...
Funksjonen `square` fungerer kun på faktiske tall. 
