...
## Program for dagen {.flexbox .vleft}
...
- Data mining, inkludert webscraping og data fra APIer
- Utvikling av interaktive webapplikasjoner
- Analyse og visualisering av geodata
- Text mining og Natural Language Processing (NLP)
- Implementering av maskinlæring og statistiske modeller
- Data Envelopment Analysis (DEA)
- Og mye, mye, mer

## Hvorfor lære R?{.flexbox .vleft}

- Allsidig høynivå programmeringsspråk med gode funksjoner for å håndtere _tabulære data_ i tillegg til en fleksibel objektmodell
- God støtte for datavisualiseringer, både innebygget og via utvidelser (pakker)
- Enkelt å dokumentere analyser og gjøre dem reproduserbare (blant annet med Rmarkdown-formatet)
- Stort og aktivt utviklingsmiljø
- Lettere å lære og mestre enn Python (men er også fornuftig å lære begge)
...
* Per i dag er det flere enn 17.000 pakker tilgjengelig via CRAN
* SUV DS har utviklet egne pakker for bruk i Riksrevisjonen!
* Få pakker i dette kurset, men vi vil bruke flere pakker i neste del av kurset
...
- Vektor: `n` elementer av **samme datatype**
- Liste: `n` elementer som kan være av ulike typer, inkludert andre lister.

**Todimensjonale:**

- Matrise: ` r  *  k` elementer av **samme datatype**
- Data frame: ` r * k` elementer, kolonner kan være av forskjellig type.

## Vektorer

Lage vektorer
```{r, echo=TRUE}
# Vi lager en vektor min_vektor med tallene 1 til 5.
min_vektor  <- c(1,2,3,4,5)
# Kortversjon
min_vektor <- c(1:5)
min_vektor
```
...
Lage vektorer
```{r, echo=TRUE}
# Vi lager en sjefsvektor
sjefer <- c('Tor', 'Stig', 'Annette', 'Åse Kristin', 'Tora', 'Sverre', 'Aleksander')
sjefer
```
## Vektorer

Avgrense vektorer: se på spesifikke elementer ved å bruke `[i]`

```{r, echo=TRUE}
sjefer <- c('Tor', 'Stig', 'Annette', 'Åse Kristin', 'Tora', 'Sverre', 'Aleksander')
sjefer[1]   # Få tak i første element i en vektor
sjefer[5]   # Det femte elementet
sjefer[4:6] # Elementene 4 til og med 6
```
...
```{r, echo=TRUE}
sjefer <- c('Tor', 'Stig', 'Annette', 'Åse Kristin', 'Tora', 'Sverre', 'Aleksander')
sjefer[c(1,4,6)]  # Elementene 1, 4 og 6
length(sjefer)    # Bruk `length()` for å tak lengden på en vektor
sjefer[length(sjefer)] # Det siste elementet
```
...
Avgrense matriser: se på spesifikke elementer ved å bruke `[r, k]`
```{r, echo=TRUE}
m <- matrix(data = 1:10, ncol = 2)
m[2, 2] # Hent verdien i rad 2, kolonne 2
m[, 2]  # Hent verdiene i kolonne 2
```
...
m[1:3, 2] # Hent verdiene i rad 3 til 5 som er i kolonne 2
```

## Data frame

Data frames (datatabeller) - kan ha ulike datatyper i hver kolonne
```{r, echo=TRUE}
df <- data.frame(tall = 1:4, bokstaver = letters[1:4])
df <- data.frame(tall = 1:4, bokstaver = c('a', 'b', 'c', 'd'))
df
```

## Inspisere data
- Vi kan inspisere data frames på samme måte som  med matriser - `[r, k]`
- I tillegg er det vanlig å bruke følgende funksjoner:
  + `head()`
  + `tail()`
  + `str()`
  + `summary()`
  + `colnames()` og `rownames()`

## Inspiser data
**mtcars**: 32 biler og 11 ulike egenskaper ved de
```{r, echo=TRUE}
head(mtcars)
```
...
USArrests: Statistikk om antall arrestasjoner for overfall, drap og voldtekt per 100 000 innbyggere i 50 stater i USA i 1973. I tillegg andelen av befolkningen som bor i urbane områder.
...
mtcars$mpg[4]    # Se verdien for indeksnr 4 i kolonne mpg
```
...
```{r, echo=TRUE}
mtcars[mtcars$gear == 5, c('mpg', 'cyl')] # Velg mpg og cyl for rader hvor gear er lik 5
```
...
3. Hvilken delstat finner du i rad 3?  
4. Hva er mordraten i denne delstaten?
5. I hvilke delstater bor 66 % av populasjonen i urbane områder?

## Lister

```{r, echo=TRUE}
lista_mi <- list(tall = 1:5, bokstaver = letters[1:5])  # Liste
lista_mi
```
...
```{r, echo=TRUE}
lista_mi[[1]]    # Det første listeelementet
lista_mi[[1]][1] # Den første verdien i det første listeelementet
lista_mi[[2]][2] # Den andre verdien i det andre listeelementet
```
...
1. Sjekk ut hjelpesiden til funksjonen `sum`. Hvilke argumenter tar funksjonen?
...
## Vi skal se på {.flexbox .vleft .extraspace}
* Likhet og ulikhet
...
1 == 0    # Test for likhet
1 == 1    # Test for likhet
0 != 1    # Test for ulikhet
0 != 0    # Test for ulikhet
```
...
alder <- 18
if (alder >= 20) {
  'Kan kjøpe både øl og sprit.'
} else if (alder %in% c(18, 19)) {
  'Kan kjøpe øl, men ikke sprit.'
} else {
  'Kan verken kjøpe øl eller sprit.'
}
```
...
alder <- 25
if (alder >= 30) {
  print('30 år eller eldre')
} 
else {
  print('Yngre enn 30 år')
}
```
...
3. Skriv kode som tilordner `x` verdien `"Mindre enn 5"` hvis uttrykket `runif(1, 1, 10)` er **mindre enn** `5` og verdien `"Større enn eller lik 5"` hvis ikke. Kjør koden. Hva er verdien av `x`?
