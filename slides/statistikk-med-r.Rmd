...
**To andre mye brukte pakker i R**
...
```{r, echo=TRUE, eval=TRUE}
#range(carseats$Sales) # plus max-min for ekte range
...
Bruk datasettet om kompensasjonsordningen
...
## Et til kult eksempel bare for å vise

## Oppgaver
Bruk data om kompensasjonsordningen
...
* Ikke én løsning, avhenger av
    - hvor godt du kjenner data
    - type missing du antar at du har

## Missing data - funksjoner i R MÅ OPPDATERES

* Missing data i R: `NA`, `NaN` 
* Kan også være 99 og 0
* `visdat` - en pakke for visualisering av missing data
* `naniar`

* `is.na()`
* `anyNA()`
* `sapply(df, anyNA)`
* NA + anything gir NA (hovedregel)
...
hk <- data.frame(
  ...
  alder = c(rep(36, 10) , rep(33, 8), 104,NA, rep(29, 8), NA, 58),
  dato = rep(dato, 3),
...
dato <- as.Date('01.03.2020', format = "%d.%m.%Y")
dato

# Konverterer til datetime med as.POSIXct()
datetime <- as.POSIXct('01.03.2020 13:09', format = "%d.%m.%Y %H:%M")
datetime
...
## Oppgave
1. Les inn datasettet carseats_dirty.R med `readRDS(carseats_dirty.R)`
2. Oppgave 2
3. Oppgave 3
4. Oppgave 4
5. Oppgave 5
