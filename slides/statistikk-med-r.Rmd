...
For disse eksemplene bruker vi datasettet `Boston` fra pakken `MASS` med `medv` som _avhengig variabel_
...
<div class="notes">
- Få mer informasjon om datasettet ved å skrive `?Boston`
- Vi skal bygge en modell for å estimere median husverdi `medv`
</div>
...
<div class="notes">
- R oppgir som standard RSE, $R^2$ og F-statistic for modellen
- RMSE er et annet vanlig brukt mål for hvor god modellen er (RMSE er nesten lik RSE som oppgis av R)
- RSE og RMSE forteller oss hvor mye en observasjon i snitt avviker fra regresjonslinjen
- $R^2$ gir et standardisert (verdien er mellom 0 og 1) mål for hvor stor del av variasjonen i avhengig variabel som forklares av modellen (i multivariat lineær regresjon bruker vi en justert $R^2$)
</div>
...
<div class="notes">
p-verdien til F-testen er signifikant, altså er koeffisienten til den uavhengige variabelen signifikant forskjellig fra 0.
</div>
