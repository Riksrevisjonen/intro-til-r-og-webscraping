...
## Rask repetisjon - subsetting

**For vektorer bruker vi indeks**:

```{r, echo=TRUE}
min.vektor <- rep(1,100,2)
min.vektor[4:6]
```
...
```{r, echo=TRUE, eval=TRUE}
# Funksjonen diff kombinert med range() gir min og maks-verdi
...
Les inn datasettet `komp_oppg.RDS` med `komp_oppgave <- readRDS('komp_oppg.RDS')`
...
komp_oppg <- readRDS('C:/Users/eho/Documents/git/intro-til-r-og-webscraping/course-material/statistikk-med-r/komp_oppg.RDS')
Sales <- komp_oppg$fastekostnader/1000
```

```{r, echo=TRUE, eval=TRUE, warning = FALSE, message = FALSE}
# Hva hvis fordelingen i Sales var veldig skjev?
...
hist(log10(Sales), breaks = 15)
rug(log10(Sales), breaks = 15)
```
...
2. Hvor mange missing er det i datasettet?
3. Hvilken datatype har `variabel` og `variabel`?
