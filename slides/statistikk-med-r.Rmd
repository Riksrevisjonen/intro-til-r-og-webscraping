...
* Les inn data med argumentet `stringsAsFactors` satt til `FALSE` (fra og med R 4.0 er standardvalget endret til `FALSE` for dette argumentet!)
...
2. Hvor mange missingverdier er det i datasettet totalt?
3. Hvilken datatype har `variabel` og `tildelingsdato`?
...
* Formelobjektet spesifiserer med tegnet `~` i formen `y ~ x`
...
* Frem til versjon 4, ble alle tekstkolonner automatisk importer om `factor` og ikke `character`. For å lese inn tekstkolonner som faktorer setter du argumentet `stringsAsFactors` til `TRUE`
...
Eksempel på datasettet `Boston` fra pakken `MASS`:

Her bygger vi en modell med median husverdi `medv` som avhengig variabel og per capita kriminalitetsrate `crim` som uavhengig variabel (se `?Boston` for en oversikt over datasettet). Merk at vi tilordner modellen til objektet `lm.mod`.

Vi bruker `summary()` for å få informasjon om modellen.
...
* Vi har flere mål for hvor godt modellen passer til og forklarer data
* R oppgir som standard _residual standard error_ (RSE), _R<sup>2</sup>_ og F-statistic for modellen
* Root mean standard error (RMSE) er et annet vanlig brukt mål for hvor god modellen er (RMSE er nesten lik RSE som oppgis av R)
* RSE og RMSE forteller oss hvor mye en observasjon i snitt avviker fra regresjonslinjen
* R<sup>2</sup> gir et standardisert (verdien er mellom 0 og 1) mål for hvor stor del av variasjonen i avhengig variabel som forklares av modellen (i multivariat lineær regresjon bruker vi en justert R<sup>2</sup>)
* F-statistikken brukes for å teste hypotesen om at det er en sammenheng mellom de uavhengige variablene og den avhengige
...
Modellen er signifikant, men den forklarer lite a variasjonen i `medv`
...
## Oppdatere modeller {.small-code}

Vi kan oppdatere en eksisterende modell med funksjonen `update()`:

```{r, echo=TRUE}
# Fjerne age og indus fra modellen
mult.mod <- update(mult.mod, ~ . -age -indus)
summary(mult.mod)
```

## Samspill {.small-code}

Vi introduserer samspill med `:`. Dette kan forkortes med `*`
...
## Transformasjoner {.small-code}

For annengradsledd bruker vi `^`, men vi må sette annengradsleddet inn i `I()`:
...
Vi kan også bruke funksjonen `poly()` for høyere grads polynomer:
...
Vi er ikke kun begrenset til polynomer; her gjør vi en logtransformering:
...
```{r, echo=TRUE}
lstat <- Boston$lstat
paste('Snitt:', round(mean(lstat), 3))
paste('Std.avvik:', round(sd(lstat), 3))
```

```{r, echo=TRUE}
lstat.scaled <- as.vector(scale(lstat))
paste('Snitt:', round(mean(lstat.scaled), 3))
paste('Std.avvik:', round(sd(lstat.scaled), 3))
```

## Undersøke modeller {.flexbox .vleft}

En rask repetisjon av de viktigste forutsetningene for lineær regresjon:

* Lineære parametre
* Tilfeldig utvalg
* Ikke perfekt kolinearitet
* Feilleddet har forventing lik 0
* Feilleddet har konstant varianse gitt de øvrige uavhengige variablene (homoskedastitet)

## Multikolinearitet {.flexbox .vleft}

* Kolinearitet i modellen reduserer nøyaktighetene i regresjonskoeffisisentene
Vi kan undersøke multikolinearitet med _variance inflation factor_ eller VIF
* I R kan vi bruke funksjonen `vif()` fra pakken `car`
* Minste verdi for VIF er 1, verdier over 10 (noen mener 5) er en indikasjon på problematisk kolinearitet i data
...
## Motivasjon for logistiske modeller
...
```{r, eval=TRUE, echo=FALSE}
layout(mat = matrix(1:3, nrow = 1), heights = rep(1, 3), widths = c(2, 1, 1))
par(bg = 'transparent')
plot(
  income~balance,
  col = ifelse(default == 'Yes', 'chocolate', 'lightskyblue'),
...
```{r, eval=TRUE, echo=FALSE}
default.num <- ifelse(Default$default == 'Yes', 1, 0)

lm.mod <- lm(default.num ~ balance, data = Default)
logit.mod <- glm(default ~ balance, data = Default, family = binomial)
lm.preds <- predict(lm.mod, Default)
logit.preds <- predict(logit.mod, Default, type = 'response')

par(mfrow = c(1, 2), bg = 'transparent')

plot(
  Default$balance, default.num, col = 'gold', pch = 3,
  xlab = 'Balance', ylab = 'Sannsynlighet for default',
  main = 'Lineær modell', cex = .8
)
abline(h = 1, lty = 2)
abline(h = 2, lty = 2)
abline(coefficients(lm.mod), col = 'steelblue', lwd = 2)

plot(
  Default$balance, default.num, col = 'gold', pch = 3,
  xlab = 'Balance', ylab = 'Sannsynlighet for default',
  main = 'Logistisk model', cex = .8
)
abline(h = 1, lty = 2)
abline(h = 2, lty = 2)
lines(
  Default$balance[order(logit.preds)],
  logit.preds[order(logit.preds)],
  lwd = 2, col = 'steelblue'
)
```
...
## Eksempel: mislighold av gjeld

Eksempeldatasett hentet fra pakken `ISLR`

* `default`: en faktorvariabel for om gjelden er misligholdt eller ikke
* `student`: en faktorvariabel som indikerer om personen er student eller ikke
* `balance`: gjennomsnittlig kreditt som gjenstår på kredittkort etter innbetaling
* `income`: inntekt til kunden

## Logistisk modell {.small-code}
...
Hva er sannsynligheten for mislighold (default) når balansen på kredittkortet er 1000 dollar?

$$\hat{p}(X)=\frac{e^{\hat{\beta_0}}+\hat{\beta_1}X_1}{1+e^{\hat{\beta_0}}+\hat{\beta_1}X_1}=\frac{e^{-10.6513+0.0055\times1000}}{1+e^{-10.6513+0.0055\times1000}}=0.00576$$

```{r, echo=TRUE}
predict(logit.mod, data.frame(balance = 1000), type = 'response')[[1]]
```

## Tolkning av koeffisientene

Hva når balansen er 2000 dollar?

$$\hat{p}(X)=\frac{e^{\hat{\beta_0}}+\hat{\beta_1}X_1}{1+e^{\hat{\beta_0}}+\hat{\beta_1}X_1}=\frac{e^{-10.6513+0.0055\times2000}}{1+e^{-10.6513+0.0055\times2000}}=0.586$$

```{r, echo=TRUE}
predict(logit.mod, data.frame(balance = 2000), type = 'response')[[1]]
```

## Vurdere modellen: sensitivitet og spesifisitet

Vi kan vurdere modellen ut fra hvor god den er til å korrekt identifisere sanne positive verdier og sanne negative verdier.

```{r, echo=TRUE}
glm.preds <- predict(logit.mod, Default, type = 'response')
glm.preds <- factor(ifelse(glm.preds > .5, 'Yes', 'No'))
addmargins(table(glm.preds, Default$default, dnn = c('Estimert', 'Observert')))
```
...
Sensitivitet: $100\times\frac{100}{333}=30,03003$

Spesifisitet: $100\times\frac{9625}{9667}=99,56553$

Nøyaktighet: $100\frac{100+9625}{10000}=97,25$

Pakken `caret` er nyttig for å vurdere ulike logistiske modeller mot hverandre.
...
1. Bygg en logistisk modell med `glm()` på datasettet `Default`. Bruk `default` som avhengig variabel og alle andre variabler som uavhengige variabler. Tilordne modellen til `glm.mod`. Husk å sette argumentet `family`
2. 

# Estimere _nye_ data

## Estimering av nye data

* Vi har allerede sett hvordan vi kan finne estimert verdi med `predict()`-funksjonen
* Denne funksjonen kan også benyttes til å estimere _nye_ data
* Dette er den grunnleggende funksjonen for maskinlæring i R!
* Vi skal her se på noen veldig forenklede eksempler på bruk av `predict()` for å estimere nye data
...
4. Evaluere modellens presisjon på _testdatasettet_

## Prediksjon på nye data

Vi starter med å lage en vektor med tilfeldige tall fra en _uniform_ distribusjon.

Denne kan vi bruke for å splitte datasettet. Vi gir alle verdier som er mindre enn `0.7` verdien `TRUE` og alle andre verdien `FALSE`. Dette bør gi oss (omtrent) en 70/30 splitt av data.

```{r, eval=TRUE, echo=FALSE}
credit <- read.csv2(sprintf('%s/course-material/statistikk-med-r/credit.csv', here::here()))
```
...
Vi starter med å dele data i et **treningsdatasett** og et **testdatasett**:
...
Vi bygger så en regresjonsmodell på _treningsdata_.

```{r, echo=TRUE}
mod <- lm(medv ~ ., data = boston.trening)
```

Vi kan så bruke modellen til å estimere husverdi både for begge datasettene med `predict()`:
